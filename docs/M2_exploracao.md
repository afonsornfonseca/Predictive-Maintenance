...
A variável alvo do nosso projeto é **“Machine failure”**, que assume dois valores: 0 quando a máquina funciona normalmente e 1 quando ocorre uma falha. Para percebermos como esta variável se distribui no dataset, calculámos as frequências e representámos os resultados num gráfico circular no Kaggle, o que revelou as seguintes conclusões essenciais:

* **Forte Desequilíbrio de Classes:** O dataset apresenta 96,61% dos registos correspondentes a funcionamento normal e apenas **3,39% de falhas reais**. Isto reflete um problema típico de manutenção preditiva, onde as avarias são eventos muito raros face aos longos períodos de operação normal.
* **A Ilusão da *Accuracy*:** Este desbalanceamento influencia diretamente a forma como iremos avaliar o modelo. Utilizar apenas a *accuracy* seria enganador: um modelo inútil que previsse sempre “não falha” teria automaticamente ~96% de acerto, falhando totalmente o objetivo do projeto.
* **Adoção de Métricas Robustas:** Para contornar este problema e avaliar a real capacidade do modelo em detetar os casos raros, decidimos utilizar métricas mais adequadas para classes desbalanceadas, nomeadamente o **Recall** (focado na classe de falha), o **F1-Score** e as curvas de *Precision-Recall*.
* **Necessidade de Estratégias Específicas:** O desequilíbrio exigirá a adoção de técnicas para lidar com a minoria de dados, como o uso de pesos diferenciados para as classes ou técnicas de reamostragem. Embora ainda não tenhamos aplicado nenhuma destas abordagens nesta fase exploratória, já as identificámos como um passo crítico no desenvolvimento da modelação.

**Em resumo:** A análise da variável alvo provou que o dataset está bem estruturado e é fiel à realidade industrial. O reduzido número de falhas não é um defeito, mas uma característica do cenário de fábrica, o que guiará as nossas decisões táticas no treino e avaliação dos algoritmos nas próximas etapas.

### 1.2. Correlações Relevantes

Após a construção do mapa de correlação das variáveis numéricas, foi possível identificar relações fortes e moderadas entre alguns atributos do dataset, que foram posteriormente validadas por análise visual:

**Visão Geral do Mapa de Correlações:**
* **Tipos Específicos de Falha:** Relativamente à variável alvo (*Machine failure*), as maiores correlações ocorrem com as variáveis de falhas específicas (HDF, OSF, PWF e TWF), com valores entre 0.36 e 0.58. Sendo estas variáveis diretamente relacionadas com a definição da falha global, esta correlação elevada era expectável.
* **Sensores Operacionais Independentes:** Observa-se que o **Torque [Nm]** apresenta a maior correlação positiva com a falha (≈ 0.19), seguido pelo **Tool wear [min]** (≈ 0.10), apresentando as restantes variáveis uma correlação muito reduzida.
* **Validação por Boxplots:** Os boxplots reforçam esta análise, mostrando que as máquinas que registaram falha apresentam, em média, valores de torque e desgaste da ferramenta superiores às que não falharam.

**Análise Detalhada dos Gráficos de Dispersão:**

* **1. Rotational speed [rpm] vs. Torque [Nm]:**
  * **Padrão e Física:** Identificou-se uma correlação negativa muito forte (≈ -0.88). À medida que a velocidade aumenta, o torque diminui, formando uma estrutura quase curva e bem delimitada. Do ponto de vista físico, esta dinâmica não é aleatória; é expectável em sistemas mecânicos rotativos, onde velocidade e binário se ajustam para manter determinados níveis de potência.
  * **Impacto nas Falhas:** As falhas surgem com maior frequência em zonas de torque mais elevado, independentemente do regime de rotação. Isto reforça a ideia de que o torque representa diretamente o nível de esforço mecânico aplicado e o risco associado.
  * **Justificação:** A inclusão destas variáveis justifica-se não apenas pela estatística, mas pela sua relevância operacional e interpretação física.

* **2. Tool wear [min] vs. Torque [Nm]:**
  * **Interação e Esforço:** Apesar de maior dispersão, as falhas concentram-se onde o desgaste e o torque assumem valores mais elevados. À medida que a ferramenta se desgasta, o sistema necessita de aplicar maior força (torque) para manter o desempenho, o que representa maior stress mecânico e probabilidade de falha.
  * **A Dimensão Temporal:** Este gráfico acrescenta uma dimensão temporal crucial à análise. O desgaste é um fenómeno acumulativo, ao contrário da velocidade e do torque (que são condições instantâneas). A variável *Tool wear* permite capturar falhas resultantes de degradação gradual e não apenas de picos súbitos de esforço.

* **3. Air temperature [K] vs. Process temperature [K]:**
  * **Contexto Térmico:** Observa-se uma relação linear muito forte positiva (≈ 0.88), indicando que o contexto externo influencia diretamente as condições internas da máquina.
  * **Impacto nas Falhas:** Embora as falhas não apresentem aqui um padrão de separação tão evidente, o contexto térmico influencia os componentes mecânicos/elétricos sob esforço. É plausível que níveis elevados de torque e desgaste contribuam para o aumento da temperatura do processo, reforçando a necessidade de considerar variáveis mecânicas e térmicas em simultâneo.

**Conclusão da Análise:**
Em conjunto, os gráficos demonstram que as variáveis selecionadas representam diferentes dimensões da máquina: a velocidade e o torque caracterizam o **regime mecânico instantâneo**, o desgaste introduz a **componente acumulativa** de degradação, e as temperaturas descrevem o **contexto ambiental**. O torque surge como a variável central (elo de ligação entre esforço, desgaste e impactos térmicos), validando o mapa de correlações e reforçando a relevância preditiva destas variáveis para a modelação.
